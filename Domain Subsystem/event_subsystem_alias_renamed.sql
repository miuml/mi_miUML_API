create or replace function midom.event_subsystem_alias_renamed(
    -- Existing
...
$$
--
-- Event: The Susystems's Alias was changed.  So we need to update our sequence names
...
begin
    ...
    new_c_seq := my_domain.alias || '_' || self.alias || '_cnum_seq';
    execute 'alter sequence ' || my_schema || '.' || self.cnum_generator
        || ' rename to ' || new_c_seq;

    new_r_seq := my_domain.alias || '_' || self.alias || '_rnum_seq';
