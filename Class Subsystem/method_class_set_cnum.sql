create or replace function miclass.method_class_set_cnum(
	-- id
...
begin
	...
	-- Cnum is in range
	begin
		update miclass.class set cnum = p_new_cnum where 
			name = self.name and domain = self.domain
			returning * into strict self;
	exception
		-- Some other class is using this cnum
		when unique_violation then
			select * from miclass.class into strict dup_class where
				cnum = p_new_cnum and domain = self.domain;
			raise exception 'Cnum [%] is in use by class [%::%]',
				dup_class.cnum, dup_class.domain, dup_class.name;
	end;
end
$$
language plpgsql;
